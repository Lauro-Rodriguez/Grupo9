...
{
    [Authorize(Roles = "Master,IN_Ingresos")]
    public class IN_TRANSACCIONESController : Controller
    {
        // GET: IN_TRANSACCIONES
        ...
        public ActionResult Index(string sortOrder, string currentFilter, string searchString, int? page)
        {
            ...
            && f.PERI_CODIGO == Pericod && (f.TIPO_INGRESO=="CP" || f.TIPO_INGRESO=="IV")).Include(i => i.Periodos).Include(i => i.Proveedores);
        ...
        // GET: IN_TRANSACCIONES/Details/5
        public ActionResult Details(int id)
        {
            return View();
        }

        // GET: IN_TRANSACCIONES/Create
        public ActionResult Create(int? IdBodega, int? NumTransac)
        {
            int IdInst = Convert.ToInt32(Session["IdInst"]);
            int PeriCod = Convert.ToInt32(Session["IdPeriodo"]);

            IN_TRANSACCIONES iN_COMPRAS = db.IN_TRANSACCIONES.Find(IdInst, PeriCod, IdBodega, NumTransac,"I","IV");
            ...
            {
                ViewBag.EsNuevo = 0;
                ViewBag.UserCrea = iN_COMPRAS.USER_CREA;
                ViewBag.FechaCrea = iN_COMPRAS.FECHA_CREA; 
                LLenaCombos(iN_COMPRAS,0);
                var DetProd = db.IN_DETALLETRANSAC.Where(f => f.ID_INSTITUCION == IdInst && f.PERI_CODIGO == PeriCod && f.ID_BODEGA == IdBodega
                      && f.NUM_TRANSACCION == NumTransac).ToList();

                foreach (var Polit in DetProd)
                {
                    AddDetCompra(Polit.NUM_FILA, Polit.COD_ITEM, Polit.PORC_IVA, Polit.CANTIDAD, Polit.COSTO_UNIT,
                        Polit.SUBTOTAL, Polit.TOTAL_GENERAL, Polit.TOTAL_IVA);
                }
            }
            else
            {
                ...
                LLenaCombos(iN_COMPRAS, 1);
            }

            ViewBag.TipoTran = "I";
            ViewBag.TipoIngre = "IV";
            ViewBag.TipoPago = "EF";
        ...
        public ActionResult Create([Bind(Include = "ID_INSTITUCION,PERI_CODIGO,ID_BODEGA,NUM_TRANSACCION,TIPO_TRAN,TIPO_INGRESO,NUM_COMPRA,FECHA_TRANSAC,TIPO_MOV,TIPO_PAGO,COD_TIPO_DOC,COD_PROV,ID_UNIDOPERATIVA,AUTORIZADO_POR,USO,DETALLE,ID_OBJETO,TIPO_SRI,FECHA_DOC,NUM_DOC,FECHACAD_DOC,AUTORIZACION,NUM_CXP,ANULADO,CERRADO,USER_CREA,FECHA_CREA,USER_MODIF,FECHA_MODIF")] IN_TRANSACCIONES iN_COMPRAS, int EsNuevo)
        {
            ...
            //if (Products == null)
            //{
            //    LLenaCombos(iN_COMPRAS, EsNuevo);
            //    ViewBag.EsNuevo = 1;
            //    ViewBag.UserCrea = Session["Usuario"].ToString();
            //    ViewBag.FechaCrea = DateTime.Today.ToString("dd/MM/yyyy");
            //    return View(iN_COMPRAS);
            ...
            try
            {
                if (EsNuevo == 1)
                {
                    NumDisp = (from nd in db.IN_TRANSACCIONES
                               where nd.ID_INSTITUCION == IdInst && nd.PERI_CODIGO == PeriCod && nd.TIPO_TRAN == "I" && nd.TIPO_INGRESO == "IV"
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    foreach (var course in Products)
                    {
                        ...
                        {
                            Productos.NUM_TRANSACCION = iN_COMPRAS.NUM_TRANSACCION;
                        }
        ...
        private void LLenaCombos(IN_TRANSACCIONES iN_COMPRAS,int EsNuevo)
        {
            int IdInst = Convert.ToInt32(Session["IdInst"]);
            int PeriCod = Convert.ToInt32(Session["IdPeriodo"]);
            ...
            {
                int TM = Convert.ToInt32(iN_COMPRAS.TIPO_MOV);
                TiposT = db.IN_TIPOSTRAN.Where(f => f.ID_INSTITUCION == IdInst && f.PERI_CODIGO == PeriCod
              && f.COD_TIPOTRAN == TM).ToList();
                ViewBag.TIPO_MOV = new SelectList(TiposT, "COD_TIPOTRAN", "DESCRIP_TIPOTRAN");
                ViewBag.ID_BODEGA = new SelectList(Bodegas.OrderBy(o => o.NOM_BODEGA), "ID_BODEGA", "NOM_BODEGA", iN_COMPRAS.ID_BODEGA);
                ViewBag.USO = new SelectList(Items, "TIPO_USO", "DESRIPCION",iN_COMPRAS.USO);
                ViewBag.COD_TIPO_DOC = new SelectList(TipDoc.OrderBy(o => o.DESCRIPCION), "COD_TIPO_DOC", "DESCRIPCION",iN_COMPRAS.COD_TIPO_DOC.Trim());
                ViewBag.COD_PROV = new SelectList(Proveed.OrderBy(o => o.NOMBRE), "COD_PROV", "NOMBRE",iN_COMPRAS.COD_PROV);
                ViewBag.AUTORIZADO_POR = new SelectList(Proveed.OrderBy(o => o.NOMBRE), "COD_PROV", "NOMBRE",iN_COMPRAS.AUTORIZADO_POR);
            }
        }
        ...
        // GET: IN_TRANSACCIONES/Edit/5
        public ActionResult Edit(int id)
        {
            return View();
        }

        // POST: IN_TRANSACCIONES/Edit/5
        [HttpPost]
        ...
        // GET: IN_TRANSACCIONES/Delete/5
        public ActionResult Delete(int id)
        {
            return View();
        }

        // POST: IN_TRANSACCIONES/Delete/5
        [HttpPost]
        ...
        public ActionResult Print(int id,string TipoTran,string TipoIngre)
        {
           ...
           //               select a).ToList();
            ...
            string path = Path.Combine(Server.MapPath("~/Reportes/Inventarios"), "RepIngreso2.rdlc");
